function CSS3MultiColumn() {
    ...
    function processElement(affectedElement, column_count, column_width, column_gap, column_rule) {
        ...
        if (affectedElement.clientWidth && affectedElement.clientWidth != 0) {
            ...
            if (affectedElement.scrollWidth) {
                var borderWidth;
                var padding;
                if (affectedElement.currentStyle) {
                    padding = parseInt(affectedElement.currentStyle.paddingLeft.replace(/[\D]*/gi, "")) + parseInt(affectedElement.currentStyle.paddingRight.replace(/[\D]*/gi, ""))
        ...
        if (!column_width || column_width == "auto") {
            ...
            if (!column_count || column_count == "auto") {
        ...
        for (var i = 1; i < column_count && elem; i++) {
            ...
            while (pn && pn.id != elem.id) {
                var n = pn.firstChild;
                while (n) {
                    if ((n.nodeType == 1 && n.childNodes.length == 0) || (n.nodeType == 3 && n.nodeValue.replace(/[\u0020\u0009\u000A]*/, "") == "")) {
                        pn.removeChild(n);
                        n = pn.firstChild
                    } else {
                        n = n.nextSibling
            ...
            if (strippedLine) {
                ...
                if (splitE && (document.defaultView && document.defaultView.getComputedStyle(splitE, "").getPropertyValue("text-align") == "justify") || (splitE.currentStyle && splitE.currentStyle.textAlign == "justify")) {
        ...
        clearFloatDiv.style.clear = "left";
...
CSS3Utility.prototype.cssQuery = function () {
    ...
    var y = 3;
    var s = 9;
    var p = /MSIE/.test(navigator.appVersion), d;
    var m = {};
    ...
    function c(N, S) {
        ...
        var J = arguments.callee.caching && !S;
        S = (S) ? (S.constructor == Array) ? S : [S] : [document];
        d = false;
        var T = x(N).split(",");
        var Q = [];
        for (var P in T) {
    ...
    var k = (p) ? function (J) {
        ...
        return J.mimeType == "XML Document"
    } : function (J) {
        if (J.nodeType == s) {
            J = J.documentElement
        }
        return J.localName != "HTML"
    };

    function x(J) {
        return J.replace(F, a).replace(D.ALL, D.ID).replace(G, b)
    }
    ...
    var j = {
        link: function (K) {
            for (var J = 0; J < document.links; J++) {
                if (document.links[J] == K) {
                    return true
                }
            }
        }, visited: function (J) {
            ...
            return !l(J)
        }, "last-child": function (J) {
            return !h(J)
        }, root: function (K) {
            var J = K.ownerDocument || K.document;
            return Boolean(K == J.documentElement)
        }, empty: function (K) {
            for (var J = 0; J < K.childNodes.length; J++) {
                if (q(K.childNodes[J]) || K.childNodes[J].nodeType == y) {
                    return false
                }
            }
            return true
    ...
    function D(J, L, K) {
        ...
        i(z, new Function("element", "return " + M))
    }

    D.prototype.toString = function () {
        return D.PREFIX + this.id
    };
    D.PREFIX = "@";
    D.ALL = /\[([^~|=\]]+)([~|]?=?)([^\]]+)?\]/g;
    D.ID = function (J, K, M, L) {
        return new D(K, M, L)
    };
    function r(R, L, J) {
        var K = "";
    ...
    var n = (p) ? function (K, J) {
        return (J == "*" && K.all) ? K.all : K.getElementsByTagName(J)
    } : function (L, J, K) {
        return (K) ? L.getElementsByTagNameNS("*", J) : L.getElementsByTagName(J)
    };
    ...
    function u(K, J, L) {
        ...
        return (J == "*") ? q(K) : (d) ? (K.tagName == J) : (K.tagName == J.toUpperCase())
    }

    var f = (p) ? "scopeName" : "prefix";
    ...
    function q(J) {
        return Boolean(J.nodeType == t && J.tagName != "!")
    }
    ...
    if ("i".replace(/i/, function () {
        ...
        var I = function (N, M) {
            var K, L = "", J = this;
            while ((K = N.exec(J))) {
                L += J.slice(0, K.index) + M(K[0], K[1], K[2], K[3], K[4]);
                J = J.slice(K.lastIndex)
            }
            return L + J
        };
        String.prototype.replace = function (K, J) {
            this.replace = (typeof J == "function") ? I : C;
            return this.replace(K, J)
        }
    }
    return c
}();
...
CSS3Utility.prototype.getPseudoCssRules = function (f, g) {
    ...
    var e = 0;
    while (d) {
        var h = g.substr(0, g.substr(0, g.indexOf(d[0])).lastIndexOf("{"));
        var a = h.substr(h.lastIndexOf("}") + 1).replace(/^\s*|\s*$/g, "");
        this.cssRules[e] = new Object();
        this.cssRules[e].selectorText = a;
        this.cssRules[e].property = f;
        this.cssRules[e].value = d[1].replace(/(\r?\n)*/g, "");
        e++;
        d = c.exec(g)
    }
};
CSS3Utility.prototype.randomId = function () {
    var b = "";
    for (var a = 0; a < 6; a++) {
        b += String.fromCharCode(97 + Math.floor((Math.random() * 24)))
    }
    return b
};
CSS3Utility.prototype.debug = function (b) {
    var a = document.getElementById("debugOutput");
